...
const readPosts: NextApiHandler = async (req, res) => {
  try {
    const { limit, pageNo, skip, includeDrafts, search, timeframe, month, year } = req.query as {
      limit: string;
      pageNo: string;
      skip: string;
    ...
    console.log("DEBUG /api/posts: query includeDrafts =", includeDrafts);
    console.log("DEBUG /api/posts: token exists =", !!token);
    console.log("DEBUG /api/posts: token role =", token?.role);
    console.log("DEBUG /api/posts: isAdmin =", isAdmin);
    console.log("DEBUG /api/posts: shouldIncludeDrafts =", shouldIncludeDrafts);
